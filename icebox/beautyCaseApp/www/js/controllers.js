angular.module('beautyCase.controllers', [])
      ...
      $scope.createUser = function () {
        ...
        $rootScope.auth.$createUser(email, password, function (error, user) {
          if (!error) {
            ...
            $window.location.href = ('#/beautycase/list');
          }
          else {
            $rootScope.hide();
...
// SCAN CONTROLLER
.controller('myScanCtrl', function($rootScope, $scope, $window, $ionicModal, $firebase) {
  $rootScope.show("Please wait... Scanning");
  $scope.list = [];
  var beautyCaseRef = new Firebase($rootScope.baseUrl + escapeEmailAddress($rootScope.userEmail));
  beautyCaseRef.on('value', function(snapshot) {
  ...
  $scope.deleteItem = function(key) {
    ...
    beautyCaseRef.child(key).remove(function(error) {
      if (error) {
        $rootScope.hide();
        $rootScope.notify('Oops! something went wrong. Try again later');
      } else {
